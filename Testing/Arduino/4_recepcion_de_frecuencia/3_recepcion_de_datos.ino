...
class ServerCallbacks : public NimBLEServerCallbacks {
  ...
  void onDisconnect(NimBLEServer* server) {
    ...
    Serial.println("Celular desconectado. Advertising se reinicia en loop.");
  }
};

float readMicLevel(int &peakToPeakOut) {
  ...
  float hf = map(peakToPeak, 0, 250, 20, 95);
...
void loop() {
  ...
  int peakToPeak = 0;
  float hf = readMicLevel(peakToPeak);
